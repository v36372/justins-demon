...
var _updateUpcomingMatch = async function() {
  const db = getDb().db();
  const hltv = getHLTV();
  ...
  var res = await hltv.getMatch({id: target.id}).catch(eHandler)
...
var _crawlUpcomingMatches = async function() {
  const db = getDb().db();
  const hltv = getHLTV();

  var matches = await hltv.getMatches().catch(eHandler)
...
var _updateTeam = async function() {
  const db = getDb().db();
  const hltv = getHLTV();
...
var _updateSeries = async function() {
  const db = getDb().db();
  const hltv = getHLTV();
...
var _updateMapStats = async function() {
  const db = getDb().db();
  const hltv = getHLTV();
...
var _updateSeriesPlayerStats = async function() {
  const db = getDb().db();
  const hltv = getHLTV();
  ...
  var getPlayerStat = function* () {
    for(var i = 0;i< 10;i++) {
      if (i < 5) {
        yield hltv.getPlayerStats({id: match.match.players.team1[i].id, startDate: f, endDate: t, rankingFilter: TOP_50}).catch(eHandler)
      } else {
        yield hltv.getPlayerStats({id: match.match.players.team2[i-5].id, startDate: f, endDate: t, rankingFilter: TOP_50}).catch(eHandler)
      }
    }
  }
...
var _updateSeriesTeamExtraStats = async function() {
  const db = getDb().db();
  const hltv = getHLTV();
  ...
  var teams = await hltv.getTeamExtraStats({startDate: f, endDate: t, rankingFilter: TOP_50}).catch(eHandler)
...
var _updateSeriesTeamStats = async function() {
  const db = getDb().db();
  const hltv = getHLTV();
...
var _crawlNewMaps = async function() {
  const db = getDb().db();
  const hltv = getHLTV();
...
var jobManager = function(){
  ...
  return {
    execute: async function(){
      ...
      busy = true
      await jobList[jobIndex].handler().catch(jobManager.errorHandler)
      busy = false;
